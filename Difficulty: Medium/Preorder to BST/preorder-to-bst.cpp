...
// A utility function to create a new tree node using the provided function
...
public:
    ...
    Node* Bst(int pre[], int size) {
        ...
        for (int i = 1; i < size; ++i) {
            ...
            // Keep popping the stack until we find the right parent for pre[i]
            while (!s.empty() && pre[i] > s.top()->data) {
                temp = s.top();
                s.pop();
            }

            // If the new value is smaller than the current node, it goes to the left
            if (temp != NULL) {
